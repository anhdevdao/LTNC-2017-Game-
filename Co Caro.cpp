...
void TextColor(int x)//Chon mau chu
{
  	SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), x);
}

//Noi dung game
int turn(int i = 0) {// Kiem tra chan le de chia turn
    if (i%2 == 0) return 1;
    else return 0;
}
...
void drawOX(int x, int y){//Ve X, O theo turn
    if (a[x][y] != 'X' && a[x][y] != 'O') {
         if (turn(i) == 1) TextColor(11); else TextColor(12);
         a[x][y] = OX(i);
         cout << a[x][y];
         i+=1;
    }
...
void gameInfo() {
    ...
    //Trang tri
    cout << "     M, E: chon X/O.";
    TextColor(12); cout << "           DAU VOI NGUOI";
    Goto(0,4); TextColor(11);cout << "Nguoi Choi 1"<< endl << "A,D,W,S"<< endl << endl;
               cout << "Nguoi choi 2"<< endl << "Phim di "<< endl << "chuyen"<< endl << endl;
    TextColor(14); cout << "p/s:"<< endl << "Chan dau,"<< endl << "chan duoi "<< endl; 
	TextColor(12); cout << "khong"; TextColor(14); cout << " tinh"<< endl << endl;
...
void win(){
    ...
    if ((Owin()==1)){
        ...
        if (key=='r' || key=='R') replay();
            ...
            if (key=='q' || key=='Q') quit(); else beginning(50);
    }
    else
        if ((Xwin()==1)){
...
void keybroad(){ //Nhan thong tin tu ban phim
    key=getch();
    ...
    if (key=='e' || key=='E' || key=='m' || key=='M') drawOX(n,m); else
    if (key=='r' || key=='R') replay();
    if (key=='b' || key=='B') beginning(50);
}
...
void beginning(int n){
	...
	cout << " _________        _______     "					<< endl
         ...
         << "     ---------  ----       ---    " 				<< endl;
	TextColor(9);
	cout << "_____  ______    __________"				<< endl
		 << "\\\\   \\/ _____\\  //    __   \\"     		<< endl
  		 << " \\\\    /-----   \\\\   /  \\   \\"		<< endl
         << "  \\\\   \\         \\\\  \\   \\   \\"	<< endl
 		 << "   \\\\   \\         \\\\  \\___/   \\"	<< endl
    ...
    cout<<char (Select);
    Sleep(200);
    if (Select== '1') humanVsHuman(); else
    if (Select== '2') tutorial(1); else
    if (Select== '3') quit(); else beginning(0);
    TextColor(7);
